...
class CommandProcessor:
    ...
    async def _route(self, text: str, low: str) -> dict[str, Any]:
        ...
        # ---- open / close app ----
        m = re.match(r"(?:please )?open (?:the )?(.+)", low)
        ...
        m = re.match(r"(?:please )?(?:close|quit|kill) (?:the )?(.+)", low)
        if m:
            app = m.group(1).strip().rstrip("?.!")
        ...
        # ---- research / search ----
        m = re.search(r"\b(?:research|search for|look up|find info on)\s+(.+)", low)
    ...
    async def _fallback(self, text: str, low: str) -> dict[str, Any]:
        """Best-effort handling of unrecognized commands."""
        # If it looks like a question/topic, offer research; otherwise guide.
        if len(text.split()) >= 2 and self.a.get("research"):
            import asyncio

            asyncio.create_task(self.a["research"].research(text))
            return {"ok": True, "intent": "fallback_research",
                    "response": f"I didn't recognize that as a command, so I'm researching "
                                f"'{text[:50]}' for you."}
        return {"ok": False, "intent": "unknown",
                "response": "I didn't understand that. Try: 'open chrome', 'add task buy milk', "
                            "'remind me in 20 minutes to stretch', or 'system stats'."}
